...
**底片** — 按拍摄时间或按胶卷分类浏览，支持自定义文件夹把多卷归到一起。导入的照片拼接成一整张长条底片，可以横向拖动；长条能切分成多个片段，每段独立标记拍摄地点，分段可随时增删（删除时相邻分段自动接上）。片边常驻胶卷型号，每格画面下方是底片编号（`1 / 1A / 2 / 2A…`）。

照片**不裁切**：横向画幅铺满格子，竖着拍的会转 90° 后铺满 —— 135 的画幅永远是横的，竖着拍的照片在底片上本来就是躺着的，转过来既是它真实的样子，也保住了全部画面。

**摄影日历** — 月历索引胶卷档案，全年柱状图统计每月拍摄量。

**相机仓库** — 登记机身与镜头。两条路径：**从相册选一张**，或者**先挑一个线稿占位**，之后随时换成真图。线稿有单反 / 旁轴 / 双反 / 傻瓜机 / 镜头五种。登记过的机身和镜头会出现在胶卷编辑页的选项里，点一下就填上。


**胶卷仓库** — 库存按过期时间由近到远排序，临近过期优先展示。新增时可以从 **29 款热门胶卷预设**里点选，型号、规格、ISO 一次填好。**同一款胶卷的不同批次分开列出** —— 过期时间是批次标识，2023 年买的一批和 2025 年又买的一批到期时间不同，合并了就看不出哪批该先用。只有型号、规格、过期时间三者全一样才累加数量。
