...
<p align="center">
  ...
  <img src="docs/05-cutout.jpg" width="30%">
</p>
...
**相机仓库** — 登记机身与镜头。拍照或从相册选图后**点一下主体就能抠掉背景**，用的是端上跑的 MediaPipe 模型（模型打进 APK，不联网也不上传）。

**胶卷仓库** — 库存计数，按过期时间由近到远排序，临近过期优先展示。
...
导入来源（扫描仪 / 相机翻拍，并可另填具体型号）、冲洗店、胶卷型号、相机型号、冲洗日期、胶卷规格（135 / 120）。**胶卷型号与相机型号都是自由文本，不受预设限制。**
